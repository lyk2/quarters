...
\tikzstyle{block2} = [draw, fill=blue!20, rectangle, minimum height=1em, minimum width=2em, text centered]
...
% Place nodes  
\node [block2](account){Creates Account};
\node [block2, below of=account](delete){Deletes Account};
\node [block2, below of=delete](login){Login};
\node [block2, below of=login](logout){Logout};
\node [block2, below of=logout](create){Creates House};
\node [block2, below of=create](join){Joins House};
\node [block2, below of=join](leave){Leaves House};
\node [block2, below of=leave](upload){Uploads File};
\node [block2, below of=upload](submit){Submits Maintenance Request};
\node [block2, below of=submit](update){Updates Maintenance Request};
\node [block2, below of=update](chore){Adds Chore};
\node [block2, below of=chore](chat){Initiates Chat};
\node [block2, below of=chat](message){Sends Message};
\node [block2, below of=message](post){Adds Post};
\node [block2, below of=post](comment){Adds Comment};
\node [block2, below of=comment](funds){Creates Record of Transaction};
\node [block, left of=submit, xshift=-5cm](user){User};
\node [block, right of=join, xshift=6cm](database){Database};
\node [block, right of=message, xshift=6cm](server){Web Server};
...
The web application shall work on the last three releases of the five most popular web browsers (Chrome, Firefox, Internet Explorer, Opera, Safari). The web application shall interface with PayPal to handle monetary transactions between users. The web application shall interface with Google Sign-In and Facebook Login to enable users to login with social media accounts. The details of the communication standards/protocols will be outlined in the Design Document after implementation is completed.
...
Only the user has access to edit their own personal stored information and choose what information of their profile is visible to other users. Users have access to view other users' profiles. Only the landlords and tenants belonging to the same property can view the property's group and add content to the property's group.
